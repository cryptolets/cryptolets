...
//=============================================================================
// Function: main
//   Test the bitshift() function using data from CSV files
//-----------------------------------------------------------------------------
CCS_MAIN(int argc, char **argv)    // required for sc verify flow in Catapult
{
  ...
  for (vector<STIMULUS_TYPE>::iterator it = samples.begin(); it != samples.end(); ++it) {
    ...
    stimulus_element.o_sample = CCS_DESIGN(bitshift)(
      stimulus_element.a_sample,
      stimulus_element.k_sample
    );
...
int ReadCSV_Samples(string filename, samplesVector_t &samples)
{
  ...
  // CSV file is expected to have 2 columns: a_sample,k_sample
  assert(CsvParser_getNumFields(header)==2);
  ...
  while ((row = CsvParser_getRow(csvparser)) ) {
    ...
    stimulus_element.k_sample = std::stoi(rowFields[1]);

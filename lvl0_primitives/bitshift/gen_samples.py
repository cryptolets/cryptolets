...
def bitshift_ref(a, k, bitwidth, bitshift_direction):
    mask = (1 << bitwidth) - 1
    if bitshift_direction == "BITSHIFT_LEFT":
        return (a << k) & mask
    if bitshift_direction == "BITSHIFT_RIGHT":
        return (a >> k) & mask
...
def generate_samples(bitwidth, total_samples, seed=42):
    ...
    if num_random > 0:
        ...
        for i in range(num_random):
            ...
            k = random.randint(0, bitwidth)
            samples.append((a, k))

    return samples

def write_csv_files(samples, bitwidth, samples_path=None, golden_path=None, bitshift_direction=None):
    # default paths
    ...
    with samples_file.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["a_sample", "k_sample"])
    ...
    with golden_file.open("w", newline="") as f:
        ...
        for a, k in samples:
            writer.writerow([bitshift_ref(a, k, bitwidth, bitshift_direction)])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bitshift-direction", type=str, help="")
    args = parser.parse_args()

    samples = generate_samples(args.bw, args.n)
    write_csv_files(samples, args.bw, args.samples_file, args.golden_file, bitshift_direction=args.bitshift_direction)

...
from framework.codegen import gen_catapult_design_tcl, gen_catapult_kernel_tcl, gen_params_h
from framework.helper import flatten_sweep, get_design_dir_name
from framework.helper import get_license_info
from framework.samples import call_gen_samples
...
def package_catapult_rtl(kernel, design_name, design_build_dir):
    ...
    text = (solution_dir / f"concat_rtl.v").read_text()

    # Catapult mangles template args into the module name, e.g. l0_add_16_false_32.
    ...
    dst = design_build_dir / "package" / f"{kernel}.v"
    dst.parent.mkdir(parents=True, exist_ok=True)
    dst.write_text(text.replace(top, kernel))
    return dst
...
def run(kernel, threads, threads_per_process, sweep, run_only, dry_run, gui_mode):
    ...
    enums = yaml.safe_load(Path(root_dir, 'framework', 'enums.yaml').read_text())

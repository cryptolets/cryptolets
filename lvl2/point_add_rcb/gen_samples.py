...
from utils.field_helpers import (
    ShortWeierstrass, get_field_const, to_mont, from_mont,
    get_q_prime, get_mu, modadd
)

from lvl2.common.padd_sw_models import point_add_sw_rcb_ref
...
def write_csv_files(
    ...
    b3 = modadd(modadd(b, b, q), b, q) # 3*b mod q
    ...
    for i in range(total_samples):
        ...
        P1_proj = E.aff_to_proj(P1)
        P2_aff = P2

        # Convert whole tuples to Montgomery
        if is_modmul_mont:
            P1_mont = to_mont(P1_proj.as_tuple(), q)
            P2_mont = to_mont(P2_aff.as_tuple(), q)
            ...
            samples.append((
                *P1_proj.as_tuple(), *P2_aff.as_tuple(), q, mu, 
                b3
            ))

        golden_proj = point_add_sw_rcb_ref(P1_proj, P2_aff, b3, q)
        ref_aff = E.add(P1, P2) # use affine point add for reference (sanity check)
        golden_aff = E.proj_to_aff(golden_proj)
        ...
        if is_modmul_mont:
            golden_proj_mont = to_mont(golden_proj.as_tuple(), q)
            goldens.append(golden_proj_mont)
        else:
            goldens.append(golden_proj.as_tuple())

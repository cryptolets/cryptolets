...
//=============================================================================
// Function: main
//   Test the point_add_rcb() function using data from CSV files
//-----------------------------------------------------------------------------
CCS_MAIN(int argc, char **argv)    // required for sc verify flow in Catapult
{
  ...
  for (vector<STIMULUS_TYPE>::iterator it = samples.begin(); it != samples.end(); ++it) {
    ...
    stimulus_element.o_sample = CCS_DESIGN(point_add_rcb)(
    ...
    , stimulus_element.field_b3_sample
#endif
    );
...
int ReadCSV_Samples(string filename, samplesVector_t &samples)
{
  ...
  while ((row = CsvParser_getRow(csvparser)) ) {
    ...
    stimulus_element.q_sample = parse_ac_int<wide_t::width>(rowFields[5]);

  #if MODMUL_TYPE == MODMUL_TYPE_BARRETT
    stimulus_element.mu_sample = parse_ac_int<wide_2x_t::width>(rowFields[6]);
  #else
    stimulus_element.q_prime_sample = parse_ac_int<wide_t::width>(rowFields[6]);
  #endif

    stimulus_element.field_b3_sample = parse_ac_int<wide_t::width>(rowFields[7]);

...
from lvl2.common.padd_sw_models import point_double_sw_ref
...
def write_csv_files(
    ...
    for i in range(total_samples):
        P0 = E.random_point()
        P0_jac = E.aff_to_jac(P0)

        # Convert whole tuples to Montgomery
        if is_modmul_mont:
            P0_mont = to_mont(P0_jac.as_tuple(), q)
            samples.append((*P0_mont, q, q_prime, to_mont(a,q)))
        else:
            samples.append((*P0_jac.as_tuple(), q, mu, a))

        golden_jac = point_double_sw_ref(P0_jac, q, a)
        ref_aff = E.add(P0, P0) # use affine point double for reference (sanity check)
        ...
        assert E.is_on_curve(P0) and E.is_on_curve(golden_aff)
    ...
    with samples_file.open("w", newline="") as f:
        ...
        samples_header = ["X1", "Y1", "Z1", "q_sample", redc_header_name, "field_a_sample"]

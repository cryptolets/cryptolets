...
// Define modmul type (rn just montgomery)
...
wide_t modsq_core(const wide_t x, const wide_t q, const wide_t q_prime) {
#if MODMUL_TYPE == MODMUL_TYPE_MONT
...
EC_point_J point_dbl_2009_l_a_0(
    ...
    EC_point_J result;
    wide_t A  = modsq_core(P0.X, q, q_prime)        ; // A = X1^2
    wide_t B  = modsq_core(P0.Y, q, q_prime)        ; // B = Y1^2
    wide_t C  = modsq_core(B, q, q_prime)           ; // C = B^2
    wide_t t0 = modadd_core(P0.X, B, q)                  ; // t0 = X1+B
    wide_t t1 = modsq_core(t0, q, q_prime)          ; // t1 = t0^2
    ...
    wide_t F  = modsq_core(E, q, q_prime)           ; // F = E^2
    ...
    t6        = moddouble_core(t6, q)                    ; // t6 = t6+t6
    wide_t t7 = modmul_core(E, t5, q, q_prime)      ; // t7 = E*t5
    result.Y  = modsub_core(t7, t6, q)                   ; // Y3 = t7-t6
    wide_t t8 = modmul_core(P0.Y, P0.Z, q, q_prime) ; // t8 = Y1*Z1
...
EC_point_J point_dbl_2009_l_a_3(
    ...
    wide_t gamma = modsq_core(P0.Y, q, q_prime)              ; // gamma = Y1^2
    wide_t beta  = modmul_core(P0.X, gamma, q, q_prime)      ; // beta = X1*gamma
    wide_t t0    = modsub_core(P0.X, delta, q)               ; // t0 = X1-delta
    wide_t t1    = modadd_core(P0.X, delta, q)               ; // t1 = X1+delta
    wide_t t2    = modmul_core(t0, t1, q, q_prime)           ; // t2 = t0*t1
    wide_t alpha = moddouble_core(t2, q)                     ; // alpha = t2+t2
    alpha        = modadd_core(t2, alpha, q)                 ; // alpha = t2+alpha
    wide_t t3    = modsq_core(alpha, q, q_prime)             ; // t3 = alpha^2
    ...
    wide_t t6    = modsq_core(t5, q, q_prime)                ; // t6 = t5^2
    ...
    wide_t t10   = modsq_core(gamma, q, q_prime)             ; // t10 = gamma^2
    wide_t t11   = moddouble_core(t10, q)                    ; // t11 = t10+t10
    t11          = moddouble_core(t11, q)                    ; // t11 = t11+t11
    t11          = moddouble_core(t11, q)                    ; // t11 = t11+t11
    wide_t t12   = modmul_core(alpha, t9, q, q_prime)        ; // t12 = alpha*t9
...
EC_point_J point_dbl_2009_l_a_var(
    ...
    EC_point_J result;
    wide_t XX   = modsq_core(P0.X, q, q_prime)        ; // XX = X1^2
    wide_t YY   = modsq_core(P0.Y, q, q_prime)        ; // YY = Y1^2
    wide_t YYYY = modsq_core(YY, q, q_prime)          ; // YYYY = YY^2
    wide_t ZZ   = modsq_core(P0.Z, q, q_prime)        ; // ZZ = Z1^2
    ...
    wide_t t4   = modsq_core(ZZ, q, q_prime)          ; // t4 = ZZ^2

#if FIELD_A == A2
    wide_t t5   = moddouble_core(t4, q)               ; // t5 = a*t4; a=2
#else
#if Q_TYPE == FIXED_Q
    wide_t t5   = modmul_mont_const(field_a, t4, q, q_prime) ; // t5 = a*t4
#else
    wide_t t5   = modmul_core(field_a, t4, q, q_prime) ; // t5 = a*t4
#endif
#endif
    ...
    wide_t t7   = modsq_core(M, q, q_prime)           ; // t7 = M^2
    ...
    t10         = moddouble_core(t10, q)              ; // t10 = t10+t10
    wide_t t11  = modmul_core(M, t9, q, q_prime)      ; // t11 = M*t9
    result.Y    = modsub_core(t11, t10, q)            ; // Y3 = t11-t10
    wide_t t12  = modadd_core(P0.Y, P0.Z, q)          ; // t12 = Y1+Z1
    wide_t t13  = modsq_core(t12, q, q_prime)         ; // t13 = t12^2
...
EC_point_J point_add(
    ...
    if (P0.Z == 0 && P1.Z == 0)
        ...
        wide_t Z1Z1 = modsq_core(P0.Z, q, q_prime)        ; // Z1Z1 = Z1^2
        wide_t Z2Z2 = modsq_core(P1.Z, q, q_prime)        ; // Z2Z2 = Z2^2
        wide_t U1   = modmul_core(P0.X, Z2Z2, q, q_prime) ; // U1 = X1*Z2Z2
        wide_t U2   = modmul_core(P1.X, Z1Z1, q, q_prime) ; // U2 = X2*Z1Z1
        wide_t t0   = modmul_core(P1.Z, Z2Z2, q, q_prime) ; // t0 = Z2*Z2Z2
        wide_t S1   = modmul_core(P0.Y, t0, q, q_prime)   ; // S1 = Y1*t0
        wide_t t1   = modmul_core(P0.Z, Z1Z1, q, q_prime) ; // t1 = Z1*Z1Z1
        wide_t S2   = modmul_core(P1.Y, t1, q, q_prime)   ; // S2 = Y2*t1
        ...
        if (U1 == U2 && S1 == S2) {
            ...
            wide_t I    = modsq_core(t2, q, q_prime)               ; // I = t2^2
            wide_t J    = modmul_core(H, I, q, q_prime)            ; // J = H*I
            wide_t t3   = modsub_core(S2, S1, q)                   ; // t3 = S2-S1
            wide_t r    = moddouble_core(t3, q)                    ; // r = 2*t3
            wide_t V    = modmul_core(U1, I, q, q_prime)           ; // V = U1*I
            wide_t t4   = modsq_core(r, q, q_prime)                ; // t4 = r^2

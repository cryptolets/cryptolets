...
echo "------------------------------------------------"
if [[ -n "${DRY_RUN}" || -n "${GEN_ONLY}" ]]; then
  echo "WARNING: Dry run or gen-only enabled. No runs will be performed."
else
  echo "WARNING: This will take large amounts of time to run."
fi
echo "------------------------------------------------"
echo "CORE = ${CORE}"

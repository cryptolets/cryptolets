...
Subtraction of unsigned integers, `z = x - y`, with a signed result. In the cryptographic context the inputs are field elements or integers that are never negative, so unsigned inputs avoid a wasted sign bit. The result is one bit wider and signed: the extra bit is the borrow out of the subtraction, so the sign costs no extra hardware.
...
The samples are the edge cases (`0`, the maximum and the midpoint, in every pairing) plus random operand pairs. The random pairs are spread across every bitwidth up to `bitwidth`, so small values are covered as well as full-width ones, and about half of the differences are negative. The golden output is the exact signed integer difference, which is computed in the reference library.

...
The samples are the edge cases (`0`, the maximum and the midpoint, in every pairing) plus random operand pairs. The random pairs are spread across every bitwidth up to `bitwidth`, so small values are covered as well as full-width ones. The golden output is the exact integer sum with carry included, which is computed in the reference library.

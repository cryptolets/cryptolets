#include "l0_int_cmul_top.h"
#include "tb_helper.h"

// The output width follows the constant and output type, as the implementation defines it
using consts = l0_int_cmul_consts<BITWIDTH, CMUL_CONST, CMUL_OUTPUT_TYPE>;

vector<string> run_per_row(vector<string>& samples_row) {
  ac_int<BITWIDTH, false> x = parse_ac_int<BITWIDTH>(samples_row[0]);
  ac_int<consts::OUT_WIDTH, false> result;
  CCS_DESIGN(l0_int_cmul_top) dut;
  dut.run(x, result);
  return {result.to_string(AC_DEC)};
}

// required for sc verify flow in Catapult
CCS_MAIN(int argc, char** argv) {
  run_tb(run_per_row, argv[1], argv[2]); // samples_file, output_file
  CCS_RETURN(0);
}
